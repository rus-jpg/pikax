...
import { getPikaApi, PIKA_APIS, resolveExample } from "@/lib/v2/pika-apis";
...
export const Route = createFileRoute("/pika-api/$")({
  ...
  notFoundComponent: () => (
    <div className="grid h-[60vh] place-items-center text-sm text-muted-foreground">
      Model not found.{" "}
      <Link to="/pika-api" className="ml-1 underline">
        Back to API list
      </Link>
    </div>
  ),
  errorComponent: ({ error }) => (
    <div className="grid h-[60vh] place-items-center text-sm text-destructive">
      {error.message}
    </div>
  ),
});
...
function ApiDetail() {
  ...
  const copySchema = async () => {
    try {
      await navigator.clipboard.writeText(api.schemaUrl);
      setCopied(true);
      ...
      <div className="mt-4 grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <aside className="lg:sticky lg:top-20 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto">
          ...
          <nav className="mt-2 flex flex-col gap-0.5">
            {PIKA_APIS.map((other) => {
              const active = other.slug === api.slug;
              return (
                <Link
                  key={other.slug}
                  to="/pika-api/$"
                  params={{ _splat: other.slug }}
                  className={cn(
              ...
              >
                {copied ? (
                  ...
                  <>
                    ...
                    <FileJson className="h-3.5 w-3.5" /> Copy OpenAPI schema
                  </>
                )}
              </button>
            </div>
            ...
            <section className="flex flex-col overflow-hidden rounded-3xl border border-border/60 bg-card">
              ...
              <div className="flex flex-col gap-5 p-5">
                {showImages && (
                  <Field
                    label={
                      allowMultipleImages ? "Keyframes / images" : "Image"
                    }
                    hint={
                      allowMultipleImages
                  ...
                  >
                    <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                      ...
                      {images.length < maxImages && (
                        <button
                          type="button"
                          onClick={() => setImages([...images, api.cover])}
                  ...
                  <Field label="Audio">
                    {audio ? (
                      <div className="flex items-center gap-3 rounded-xl border border-border/60 bg-muted/40 p-3">
                        ...
                        <audio src={audio} controls className="h-8 flex-1" />
                        <button
            ...
            <section className="flex flex-col gap-6">
              <div className="overflow-hidden rounded-3xl border border-border/60 bg-card">
                ...
                <div className="border-t border-border/60 px-5 py-3 text-[11px] text-muted-foreground">
                  Output for the pre-filled inputs above. Edit the inputs and
                  sign in to render your own.
                </div>
              </div>
