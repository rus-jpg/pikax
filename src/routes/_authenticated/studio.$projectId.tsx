...
const STEPS_BY_KIND: Record<Skill["kind"], Step[]> = {
  image: [
...
// Per-app step copy. Overrides the generic per-kind defaults so each App's
// "How it works" panel reads as a unique recipe.
const STEPS_BY_SKILL_ID: Record<string, Step[]> = {
  // ── Photo Apps ───────────────────────────────────────────────
  "app-character-swap": [
...
function HowItWorks({ skill }: { skill: Skill }) {
  const steps = STEPS_BY_SKILL_ID[skill.id] ?? STEPS_BY_KIND[skill.kind];
